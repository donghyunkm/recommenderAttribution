...
First, each item is mapped to a vector embedding. 

Next, fixed length sequences of vector embeddings are fed into a LSTM model. 
...
We use the Movielens 1M Dataset for experiments [@maxwell2015k]. The dataset contains 1 million ratings from 6000 users on 4000 movies with timestamps for each rating.

For results shown below, we use a subset of the dataset: around 100000 training sequences. 
